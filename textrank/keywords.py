    ...
    for i in range(len_text):
        ...
        if word in _keywords:
            ...
            for j in range(i + 1, len_text):
                ...
                current_split_text = split_text[j]
                if not isinstance(current_split_text, str):
                    current_split_text = str(current_split_text, 'utf-8')
                if other_word in _keywords and other_word == current_split_text:
...
def __format_results(_keywords, combined_keywords, text):
    """
    :param _keywords: dict of keywords:scores
    """
    ...
    for word in combined_keywords:
        for match in re.finditer(word + r'+[\wА-Яа-я]*(?=[\s\.$;:,?!"\']+)', text):
            spans.append(Span(*match.span(), word))
    return spans


def _format_results(_keywords, combined_keywords, n_best):
    """
    :param _keywords:dict of keywords:scores
    ...
    for key, value in _keywords.items():
        words[" ".join(lemmatizator.lemmatize(key)).strip()] = value
    words_sorted = sorted(words, key=words.get, reverse=True)
    return words_sorted[:n_best]


def extract_keywords(text, ratio=0.2, n_best=10):
    ...
    extracted_lemmas = _extract_tokens(graph.nodes(), pagerank_scores, ratio, None)
    ...
    return _format_results(keywords, combined_keywords, n_best)
