...
def _get_labels(text, by_sentence):
    syntactic_units = _clean_text_by_sentences(text) if by_sentence \
        else _clean_text_by_word(text).values()
...
def _get_viz_data(node, positions, scores):
    viz_data = {'position': {'x': positions[node][0],
                             'y': positions[node][1]},
                'size': scores[node],
                'color': NODE_COLOR}
    return viz_data
...
def gexf_export(text, path="test.gexf", labels=None, by_sentence=True, by_word=False):
    ...
    if labels is None:
        labels = _get_labels(text, by_sentence)

    graph = _get_sentence_graph(text) if by_sentence else _get_word_graph(text)
